...
public class SQLPlugin implements FrontierPlugin {
    ...
    private static Hashtable<HashSet<String>, SQLTimes> tableNames =
            new Hashtable<HashSet<String>, SQLTimes>();
    ...
    private static synchronized void getDataSource() throws Exception {
        ...
        if (dataSource == null)
            throw new Exception("DataSource [" + Frontier.getDsName() + "] not found");
    }
    ...
    public MethodDesc fp_getMethodDesc(String name) throws Exception {
        MethodDesc ret = new MethodDesc("DEFAULT", "get", false, ((long) 60 * 60 * 24 * 7
                * 1000), "free", "public");
        return ret;
    }
    ...
    public int fp_get(DbConnectionMgr mgr, Encoder enc, String method) throws Exception {
        ...
        try {
            ...
            MDC.put("queryTime", String.valueOf(timestamp2 - timestamp));
    ...
    protected static synchronized SQLTimes getSQLTimesObject(HashSet<String> qTableNames,
        ...
        while (true) {
            if (!endmatcher.find(starttablename)) {
                if (Frontier.getHighVerbosity())
                    Frontier.Log("getQueryTableNames: no end match, adding name "
                            + str.substring(starttablename));
                tablesSet.add(str.substring(starttablename));
                break;
            }
            int endtablename = endmatcher.start();
            if (Frontier.getHighVerbosity())
                Frontier.Log("getQueryTableNames: adding name "
                        + str.substring(starttablename, endtablename));
            tablesSet.add(str.substring(starttablename, endtablename));
            if (!anothermatcher.find()) {
                if (Frontier.getHighVerbosity())
                    Frontier.Log("getQueryTableNames: no more names found");
                break;
            }
    ...
    private long isAllTable(int startfrom) throws Exception {
        ...
        if (Frontier.getHighVerbosity())
            Frontier.Log("numTables: " + numTables + ", queryTableName: " + queryTableName);
        ...
        if ((startowner > startfrom) && (queryLower.substring(startfrom,
                startfrom + 4).equals("all_"))) {
            startowner += 6;
            while (query.charAt(startowner) == ' ')
                startowner++;
    ...
    */
    public long fp_cachedLastModified() throws Exception {
        if (Frontier.getHighVerbosity())
            Frontier.Log("fp_cachedLastModified()");
        ...
        if (!queryLower.startsWith("select ")) {
            if (Frontier.getHighVerbosity())
                Frontier.Log("fp_cachedLastModified() !'select '");
            return -1;
        }
        ...
        if (startfrom == -1) {
            if (Frontier.getHighVerbosity())
                Frontier.Log("fp_cachedLastModified() startfrom==-1");
            return -1;
        }
        startfrom += 6;
        ...
        if (isAllTable(startfrom) < 0) {
            if (Frontier.getHighVerbosity())
                Frontier.Log("fp_cachedLastModified() isAllTable()<0");
            return -1;
        }
        ...
        if (times == null) {
            if (Frontier.getHighVerbosity())
                Frontier.Log("fp_cachedLastModified() times==null");
            return 0;
        }
        ...
        if (Frontier.getHighVerbosity())
            Frontier.Log("fp_cachedLastModified() last_modified: " + last_modified);
        return last_modified;
    }

    public long fp_getLastModified(long if_modified_since) throws Exception {
        if (queryTableNames == null)
            throw new Exception("SQLPlugin usage error -- fp_cachedLastModified must be called "
                    + "and return zero before calling fp_getLastModified");
