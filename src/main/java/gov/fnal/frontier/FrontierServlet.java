...
public final class FrontierServlet extends HttpServlet {
    ...
    public static String frontierVersion() {
        ...
        if (Frontier.send_stale_if_error && (idx >= 0)) {
            ...
            if (idx >= 0) {
                ...
                response.setHeader("Cache-Control",
                        "max-age=" + age + ", stale-if-error=" + cch.substring(idx, endidx));
                return;
            }
        }
        else
    ...
    private String error_payload_end(ServletOutputStream out, HttpServletRequest request,
                                     HttpServletResponse response, String descript, String check,
                                     long error_max_age) throws Exception {
        String msg = "";
        long max_age = 0;
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (Frontier.getHighVerbosity())
            Frontier.Log("FrontierServlet.java:service()");
        ...
        try {
            ...
            try {
                if (Frontier.getHighVerbosity())
                    Frontier.Log("FrontierServlet.java:service(): before Frontier(request)");
                frontier = new Frontier(request);
                if (Frontier.getHighVerbosity())
                    Frontier.Log("FrontierServlet.java:service(): after Frontier(request)");
            }
            catch (Throwable e) {
            ...
            if (frontier.noCache) {
                if (Frontier.getHighVerbosity())
                    Frontier.Log("FrontierServlet.java:service(): frontier.noCache");
                response.setHeader("Pragma", "no-cache");
            }
            setAgeExpires(request, response, frontier.max_age);
            if (Frontier.getHighVerbosity())
                Frontier.Log("FrontierServlet.java:service(): setAgeExpires: frontier.max_age: "
                        + frontier.max_age);
            ...
            try {
                ...
                if (Frontier.getHighVerbosity())
                    Frontier.Log(
                            "last_modified: " + last_modified + ": " + dateHeader(last_modified)
                                    + " if_modified_since: " + if_modified_since + ": "
                                    + dateHeader(if_modified_since));
            ...
            if (Frontier.getHighVerbosity())
                Frontier.Log("FrontierServlet.java:service(): before per payload loop");
            try {
                for (int i = 0; i < frontier.payloads_num; i++) {
                    ...
                    try {
                        ...
                        globalErrorMsg = error_payload_end(out, request, response,
                                throwableDescript(e), p.getCheck(), frontier.error_max_age);
                        return;
                    }
                    ...
                    try {
                        if ((frontier.payloads_num == 1) && (last_modified == 0)) {
                            if (response.isCommitted()) {
                                ...
                                Frontier.Log("response committed, too late to query for "
                                        + "last-modified time");
                            }
                            else {
                                ...
                                try {
                                    if (Frontier.getHighVerbosity())
                                        Frontier.Log("FrontierServlet.java:service(): Going to "
                                                + "call frontier.getLastModified()");
                                    last_modified = frontier.getLastModified(out,
                                            if_modified_since);
                                }
                                catch (Throwable e) {
                                    Frontier.Log("Error getting last-modified time: "
                                            + throwableDescript(e));
                                    globalErrorMsg = error_payload_end(out, request, response,
                                            throwableDescript(e), p.getCheck(),
                                            frontier.error_max_age);
                                    return;
                                }
                                if (response.isCommitted()) {
                                    Frontier.Log("response committed while querying last-modified"
                                            + " time, too late to use");
                                }
                                else if ((if_modified_since > 0) && (if_modified_since
                                        == last_modified)) {
                                    response.setStatus(HttpServletResponse.SC_NOT_MODIFIED);
                        ...
                        try {
                            ...
                            if ((p.rec_num == 0) && frontier.expire_empty_queries_like_errors) {
                                if (response.isCommitted()) {
                                    ...
                                    Frontier.Log("empty response, setting payload max age to "
                                            + max_age);
                                }
                                else {
                                    if (last_modified > 0) {
                                    ...
                                    setAgeExpires(request, response, frontier.error_max_age);
                                    Frontier.Log(
                                            "empty response, setting max age and payload max age "
                                                    + "to "
                                                    +
                                                    frontier.error_max_age);
                                }
                            ...
                            ResponseFormat.payload_end(out, p.err_code, p.err_msg, max_age,
                                    p.getCheck(), p.rec_num, p.full_size);
                        }
                        catch (Throwable e) {
                            if (e.getClass().getSimpleName().equals("ClientAbortException")) {
                                // client has disconnected, don't send anything else
                                Frontier.Log(
                                        "Client disconnected while processing payload " + i + ": "
                                                + throwableDescript(e));
                            }
                            else {
            ...
            Frontier.Log("stop threads=" + count_current + " msecs=" + ((new Date()).getTime()
                    - timestamp));
            ...
            if (fatalErrorThrowable != null) {
                String touchpath = getServletConfig().getServletContext().getRealPath("WEB-INF"
